...
- **Immediate workaround, no AWS wait required:** set `REMOTION_LAMBDA_FRAMES_PER_LAMBDA` (e.g. `20`) in the same place you set the other five Lambda env vars. This raises how many frames each Lambda invocation renders, which lowers how many invocations run concurrently — trading render speed for staying under a low quota. Raise the number until renders stop throttling; there's no single right value, it depends on your account's actual quota and the video's frame count. **But don't raise it without limit** — every frame in a chunk has to render inside the deployed function's timeout, so past a certain size you stop getting `Rate Exceeded.` and start getting `The main function timed out` instead (see the next entry). Frames-per-lambda and the function timeout have to be tuned together.

**`The main function timed out after NNNNNms`** (with a list of chunks that didn't finish) — the deployed function's timeout is a ceiling on the **whole render**, not on any one step. The main function fans frames out to the renderer invocations, waits for every chunk, then stitches them, all inside that one budget; each renderer separately has to finish its own chunk inside it too. So the error means the render genuinely needed longer than the function allows.

Read the chunk list in the error: it tells you how big each chunk was. `Chunk 0 (Frames 0 - 1499)` means 1500 frames were handed to one invocation — at 1080×1920 that alone needs several minutes, so a 120-second function could never have finished it.

This interacts directly with the `Rate Exceeded.` fix above, and it's easy to trade one failure for the other: raising `REMOTION_LAMBDA_FRAMES_PER_LAMBDA` to dodge a low concurrency quota makes each chunk longer, and chunk length is capped by the function timeout. Fix both ends:
- **Raise the timeout.** `npm run lambda:deploy-function` now deploys at 900 seconds (AWS's maximum) instead of 120. This costs nothing extra — Lambda bills for time actually used, so a render finishing in 90s is billed identically under either timeout. **The timeout is part of the function's name** (`...-120sec` → `...-900sec`), so this deploys a *new* function and you must repoint `REMOTION_LAMBDA_FUNCTION_NAME` in Vercel at the name the script prints. Until you do, renders keep hitting the old function and failing exactly as before.
- **Then bring frames-per-lambda back down.** With a 900s timeout you no longer need chunks that huge. For a ~4000-frame video, `REMOTION_LAMBDA_FRAMES_PER_LAMBDA=400` gives ~10 concurrent invocations — low enough for a restricted quota, small enough that each chunk finishes in a couple of minutes. Lower it further (faster renders, more concurrency) once your quota increase lands; Remotion's own unset default is 20 frames per invocation, which for that video would mean ~205 invocations.

**Renders are slow but succeeding** — Lambda scales CPU with memory, so memory is really a speed dial. `npm run lambda:deploy-function -- --memory=3008` roughly halves per-chunk time for roughly the same total cost (more MB × fewer ms). Same naming caveat as above: it's a new function, repoint `REMOTION_LAMBDA_FUNCTION_NAME`.
